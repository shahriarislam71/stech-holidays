...
export default function FlightManagement() {
  ...
  const [markup, setMarkup] = useState(8); // Default 8% markup
  ...
  const duffelUrls = {
    orders: `${DUFFEL_BASE_URL}/test/orders`,
    stays: `${DUFFEL_BASE_URL}/test/stays`,
    testOrders: `${DUFFEL_BASE_URL}/test/orders`
  };

  // Fetch flight analytics
// Fix the fetchAnalytics function
const fetchAnalytics = async () => {
  setLoading(true);
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/flights/analytics/`, {
      headers: {
  ...
  const updateMarkup = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/flights/markup/`, {
        method: 'PUT',
        headers: {
  ...
  const generateItineraryPDF = async (bookingId) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/flights/itinerary/${bookingId}/pdf/`, {
        headers: {
          'Authorization': `Token ${localStorage.getItem('authToken')}`
        }
      });
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `itinerary-${bookingId}.pdf`;
  ...
  const sendItineraryEmail = async (bookingId) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/flights/itinerary/${bookingId}/email/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Token ${localStorage.getItem('authToken')}`
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Monitor flight bookings and manage markups</p>
        </div>
        <button 
          onClick={() => window.open(duffelUrls.orders, '_blank')}
          className="flex items-center px-4 py-2 bg-[#5A53A7] text-white rounded-lg hover:bg-[#4a4488] transition"
        >
...
<p className="text-2xl font-bold text-[#445494]">
  {loading ? <span className="text-gray-400">...</span> : analytics.totalBookings || 0}
</p>
...
<p className="text-2xl font-bold text-[#445494]">
  {loading ? <span className="text-gray-400">...</span> : `${analytics.conversionRate || 0}%`}
</p>            </div>
            <div className="p-2 bg-purple-50 rounded-lg">
              <TbPercentage className="text-purple-500" size={20} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500">Current Markup</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <h2 className="text-xl font-semibold text-[#445494] mb-4">Markup Configuration</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Flight Markup Percentage
                </label>
              ...
              <p className="text-sm text-gray-500">
                This markup will be applied to all flight bookings in BDT
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-[#445494]">Top Routes</h2>
              ...
              {analytics.topRoutes.map((route, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-[#445494]">৳{route.revenue.toLocaleString()}</p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-[#445494]">Recent Bookings</h2>
              <button 
                onClick={() => window.open(duffelUrls.orders, '_blank')}
              ...
              {analytics.recentBookings.map((booking) => (
                <div key={booking.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:bg-gray-50">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {booking.departure} → {booking.arrival} • {booking.date}
                    </p>
                  </div>
...
<span className="font-bold">
  ৳{(booking.amount || 0).toLocaleString()}
</span>                    <div className="flex space-x-1">
                      <button
                        onClick={() => generateItineraryPDF(booking.id)}
                      ...
                      <button
                        onClick={() => sendItineraryEmail(booking.id)}
                        className="p-1 hover:bg-gray-100 rounded"
                        title="Send Email"
                      >
