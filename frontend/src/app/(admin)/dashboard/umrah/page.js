...
import {
  FiPackage, FiUsers, FiCalendar, FiDollarSign, FiEdit2, 
  FiTrash2, FiPlus, FiSearch, FiFilter, FiDownload,
  FiCheckCircle, FiXCircle, FiClock, FiChevronDown, 
  FiChevronUp, FiMail, FiEye, FiFileText
} from 'react-icons/fi';
  ...
  const handleStatusUpdate = async (id, newStatus, type = 'booking') => {
    try {
      ...
      if (response.ok) {
        ...
        if (type === 'booking') {
          setBookings(bookings.map(b => b.id === updatedItem.id ? updatedItem : b));
        } else {
          setPackages(packages.map(p => p.id === updatedItem.id ? updatedItem : p));
        }
                    ...
                    <tr key={pkg.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            ...
                            <div className="text-sm text-gray-500">Max: {pkg.max_people} people</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-1">
                          {pkg.includes_flight && (
                            <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">Flight</span>
                          )}
                          {pkg.includes_hotel && (
                            <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">Hotel</span>
                          )}
                          {pkg.includes_transport && (
                            <span className="px-2 py-1 bg-purple-100 text-purple-800 text-xs rounded-full">Transport</span>
                          )}
                          {pkg.includes_visa && (
                            <span className="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded-full">Visa</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                ...
                {bookings.length > 0 ? (
                  bookings.map((booking) => (
                    <tr key={booking.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">UM{booking.id.toString().padStart(6, '0')}</div>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900 font-medium">{booking.package?.title || 'Package deleted'}</div>
                        <div className="text-sm text-gray-500">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <select
                          ...
                          onChange={(e) => handleStatusUpdate(booking.id, e.target.value, 'booking')}
                          className={`text-xs leading-5 font-semibold rounded-full px-3 py-1 ${statusOptions.find(s => s.value === booking.status)?.color || 'bg-gray-100 text-gray-800'}`}
                        >
                          {statusOptions.map(option => (
                            <option key={option.value} value={option.value}>
                              {option.label}
            ...
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl mx-auto overflow-hidden transform transition-all max-h-[90vh] overflow-y-auto">
              ...
              <div className="bg-gradient-to-r from-[#5A53A7] to-[#445494] px-6 py-4">
                <h3 className="text-xl font-semibold text-white">
                  {currentPackage.id ? 'Edit Umrah Package' : 'Add New Umrah Package'}
                </h3>
              </div>
              
              {/* Modal Content */}
              <div className="p-6 space-y-6">
                <form onSubmit={currentPackage.id ? handleUpdatePackage : handleCreatePackage}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            setCurrentPackage({...currentPackage, featured_image: file});
                          }
                        }}
                        ...
                        <div className="mt-2 w-full h-48 relative rounded-md overflow-hidden">
                          <img 
                            src={
                              ...
                              currentPackage.featured_image : 
                              URL.createObjectURL(currentPackage.featured_image)
                            }
                            alt="Package Preview" 
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCurrentPackage({...currentPackage, nights: parseInt(e.target.value) || 0})}
                        required
                        min="1"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCurrentPackage({...currentPackage, days: parseInt(e.target.value) || 0})}
                        required
                        min="1"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCurrentPackage({...currentPackage, max_people: parseInt(e.target.value) || 1})}
                        required
                        min="1"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={(e) => setCurrentPackage({...currentPackage, discount_price: parseFloat(e.target.value) || 0})}
                          min="0"
                          step="0.01"
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCurrentPackage({...currentPackage, availability_start: e.target.value})}
                        required
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCurrentPackage({...currentPackage, availability_end: e.target.value})}
                        required
                      />
                    </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <label className="flex items-center space-x-2">
                          <input
                            ...
                            onChange={(e) => setCurrentPackage({...currentPackage, includes_flight: e.target.checked})}
                          />
                        ...
                        <label className="flex items-center space-x-2">
                          <input
                            ...
                            onChange={(e) => setCurrentPackage({...currentPackage, includes_hotel: e.target.checked})}
                          />
                        ...
                        <label className="flex items-center space-x-2">
                          <input
                            ...
                            onChange={(e) => setCurrentPackage({...currentPackage, includes_transport: e.target.checked})}
                          />
                        ...
                        <label className="flex items-center space-x-2">
                          <input
                            ...
                            onChange={(e) => setCurrentPackage({...currentPackage, includes_visa: e.target.checked})}
                          />
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen p-4">
            ...
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl mx-auto overflow-hidden transform transition-all max-h-[90vh] overflow-y-auto">
              ...
              <div className="bg-gradient-to-r from-[#5A53A7] to-[#445494] px-6 py-4">
                <h3 className="text-xl font-semibold text-white">
                  Umrah Booking #UM{currentBooking.id.toString().padStart(6, '0')}
                </h3>
                <div className="flex justify-between items-center mt-2">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    statusOptions.find(s => s.value === currentBooking.status)?.color || 'bg-gray-100 text-gray-800'
                  }`}>
                    {statusOptions.find(s => s.value === currentBooking.status)?.label || currentBooking.status}
                  </span>
                  <button 
                    onClick={() => setShowBookingModal(false)}
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <h4 className="text-lg font-semibold mb-4 text-gray-800 border-b pb-2">Customer Information</h4>
                    <div className="space-y-3">
                      <div>
                        <p className="text-sm text-gray-500">Full Name</p>
                        <p className="font-medium">{currentBooking.contact_name}</p>
                      </div>
                      <div>
                  ...
                  {/* Booking Details */}
                  <div>
                    <h4 className="text-lg font-semibold mb-4 text-gray-800 border-b pb-2">Booking Details</h4>
                    <div className="space-y-3">
                      <div>
                        <p className="text-sm text-gray-500">Package</p>
                        <p className="font-medium">{currentBooking.package?.title || 'Package deleted'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Price</p>
                        <p className="font-medium">BDT {currentBooking.package?.price?.toLocaleString() || '0'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Departure Date</p>
                        <p className="font-medium">
                          {currentBooking.departure_date ? new Date(currentBooking.departure_date).toLocaleDateString() : 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Travelers</p>
                        <p className="font-medium">{currentBooking.travelers}</p>
                      </div>
                      <div>
                ...
                {/* Package Inclusions */}
                <div>
                  <h4 className="text-lg font-semibold mb-4 text-gray-800 border-b pb-2">Package Inclusions</h4>
                  <div className="flex flex-wrap gap-2">
                    {currentBooking.package?.includes_flight && (
                      <span className="px-3 py-1 bg-blue-100 text-blue-800 text-sm rounded-full">Flight</span>
                    )}
                    {currentBooking.package?.includes_hotel && (
                      <span className="px-3 py-1 bg-green-100 text-green-800 text-sm rounded-full">Hotel</span>
                    )}
                    {currentBooking.package?.includes_transport && (
                      <span className="px-3 py-1 bg-purple-100 text-purple-800 text-sm rounded-full">Transport</span>
                    )}
                    {currentBooking.package?.includes_visa && (
                      <span className="px-3 py-1 bg-yellow-100 text-yellow-800 text-sm rounded-full">Visa</span>
                    )}
                  </div>
                </div>
                ...
                {currentBooking.custom_request && (
                  <div>
                    <h4 className="text-lg font-semibold mb-4 text-gray-800 border-b pb-2">Special Requests</h4>
                    <p className="text-gray-700 whitespace-pre-line">{currentBooking.custom_request}</p>
                  </div>
                )}
              </div>
              ...
              <div className="sticky bottom-0 bg-white p-4 border-t flex justify-between items-center">
                <div>
                  <select
                    ...
                    onChange={(e) => {
                      handleStatusUpdate(currentBooking.id, e.target.value, 'booking');
                      setCurrentBooking({
                        ...currentBooking,
