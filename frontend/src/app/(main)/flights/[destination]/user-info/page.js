...
const UserInfoPage = () => {
  ...
  const totalPassengers = flightData.adults + flightData.children + flightData.infants;
    ...
    switch (field) {
      ...
      case 'family_name':
        if (!value.trim()) {
          ...
          newErrors[passengerIndex][field] = "Only letters, spaces, hyphens, dots and apostrophes allowed";
        } else {
      ...
      case 'number': // Passport number
        if (!value.trim()) {
          ...
          newErrors[passengerIndex]['passport_number'] = "Invalid passport number";
        } else {
      ...
      case 'expires_on':
        if (!value) {
          ...
          newErrors[passengerIndex]['passport_expiry'] = "Passport must not be expired";
        } else {
    ...
    for (let i = 0; i < flightData.adults; i++) {
      initialPassengers.push({
        ...
        identity_documents: [{
          type: "passport",
          number: "",
          issuing_country_code: "BD", // Default to Bangladesh
          expires_on: "",
          unique_identifier: ""
        }]
      });
  ...
  const updatePassengerDocument = (passengerIndex, field, value) => {
    setPassengers(prev => prev.map((passenger, i) => {
      if (i === passengerIndex) {
        const updatedDocuments = passenger.identity_documents.map((doc, docIndex) => 
          docIndex === 0 ? { ...doc, [field]: value } : doc
        );
        return { ...passenger, identity_documents: updatedDocuments };
      }
      return passenger;
    }));
    
    // Validate document field in real-time if it's been touched
  ...
  const handleBlur = (passengerIndex, field, isDocument = false) => {
    setTouched(prev => ({
      ...prev,
      [isDocument ? `${passengerIndex}_doc_${field}` : `${passengerIndex}_${field}`]: true
    }));
  ...
  const generateUniqueIdentifier = (passengerIndex) => {
    ...
    if (document.number && document.type) {
      return `${document.type}_${document.number}_${Date.now()}_${passengerIndex}`;
    }
  ...
  const validateForm = () => {
    ...
    passengers.forEach((passenger, index) => {
      ...
      const fieldsToValidate = [
        'title', 'given_name', 'family_name', 'born_on', 'gender', 'email', 'phone_number'
      ];
      ...
      const document = passenger.identity_documents[0];
      const docFieldsToValidate = ['number', 'issuing_country_code', 'expires_on'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare order data with generated unique identifiers
      const orderData = {
        data: {
          selected_offers: [flightData.offer_id],
          passengers: passengers.map((passenger, index) => ({
            ...
            identity_documents: passenger.identity_documents.map(doc => ({
              ...
              unique_identifier: doc.unique_identifier || generateUniqueIdentifier(index)
            }))
          })),
          type: "instant",
          metadata: {
            fare_name: flightData.fare_name,
      ...
      const response = await fetch(`${apiUrl}/flights/orders/`, {
        method: "POST",
        headers: {
      ...
      const paymentParams = new URLSearchParams({
        order_id: result.duffel_order?.data?.id || result.local_order?.id,
        amount: flightData.price,
        currency: flightData.price?.split(' ')[0] || 'GBP',
        ...flightData
      });

      router.push(`/flights/payment?${paymentParams.toString()}`);
  ...
  const getMaxBirthDate = (passengerType) => {
    ...
    if (passengerType === 'adult') {
      ...
      const minChildDate = new Date(today.getFullYear() - 12, today.getMonth(), today.getDate());
  ...
  const getMinBirthDate = (passengerType) => {
    ...
    if (passengerType === 'infant') {
      const maxInfantDate = new Date(today.getFullYear() - 2, today.getMonth(), today.getDate());
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 sticky top-4">
              ...
              <div className="p-6">
                <div className="flex items-center mb-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-800">{flightData.airline}</h3>
                    <p className="text-sm text-gray-600">Flight {flightData.flight_number}</p>
                  </div>
                </div>

                <div className="flex justify-between items-center py-3 border-t border-b border-gray-100">
                  <div>
                    <p className="font-semibold text-gray-800">{flightData.departure_time}</p>
                    <p className="text-sm text-gray-500">{flightData.departure}</p>
                  </div>
                  <div className="mx-2">
                  ...
                  <div>
                    <p className="font-semibold text-gray-800">{flightData.arrival_time}</p>
                    <p className="text-sm text-gray-500">{flightData.arrival}</p>
                  </div>
                </div>
                ...
                <div className="mt-4 space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{flightData.cabin_class}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Passengers</span>
                    <span className="font-medium">{totalPassengers} traveler(s)</span>
                  </div>
                  <div className="flex justify-between pt-2 border-t border-gray-200">
                    <span className="text-gray-800 font-medium">Total Price</span>
                    <span className="text-xl font-bold text-[#5A53A7]">{flightData.price}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-2">
            <form onSubmit={handleSubmit}>
              {passengers.map((passenger, index) => (
                <div key={passenger.id} className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 mb-6">
                  <div className="p-6 bg-gray-50 border-b border-gray-200">
                    ...
                    {passenger.type === 'child' && (
                      <p className="text-sm text-gray-500 mt-1">Must be between 2 and 11 years</p>
                    )}
                    {passenger.type === 'infant' && (
                      <p className="text-sm text-gray-500 mt-1">Must be under 2 years</p>
                    )}
                  </div>
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          {errors[index]?.title && <p className="mt-1 text-sm text-red-600">{errors[index].title}</p>}
                        </div>
                        <div>
                          ...
                          {errors[index]?.gender && <p className="mt-1 text-sm text-red-600">{errors[index].gender}</p>}
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            className={`w-full px-4 py-2 border ${
                              errors[index]?.given_name ? 'border-red-500' : 'border-gray-300'
                            } rounded-lg focus:ring-2 focus:ring-[#55C3A9] focus:border-[#55C3A9] outline-none transition`}
                            placeholder="As shown on passport"
                          />
                          {errors[index]?.given_name && <p className="mt-1 text-sm text-red-600">{errors[index].given_name}</p>}
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            placeholder="As shown on passport"
                          />
                          {errors[index]?.family_name && <p className="mt-1 text-sm text-red-600">{errors[index].family_name}</p>}
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            className={`w-full px-4 py-2 border ${
                              errors[index]?.born_on ? 'border-red-500' : 'border-gray-300'
                            } rounded-lg focus:ring-2 focus:ring-[#55C3A9] focus:border-[#55C3A9] outline-none transition`}
                          />
                          {errors[index]?.born_on && <p className="mt-1 text-sm text-red-600">{errors[index].born_on}</p>}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          />
                          {errors[index]?.phone_number && <p className="mt-1 text-sm text-red-600">{errors[index].phone_number}</p>}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      <h4 className="font-medium text-gray-800 mb-4">Passport Information</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Passport Number*</label>
                          <input
                            type="text"
                            value={passenger.identity_documents[0]?.number || ''}
                            onChange={(e) => updatePassengerDocument(index, 'number', e.target.value)}
                          ...
                          />
                          {errors[index]?.passport_number && <p className="mt-1 text-sm text-red-600">{errors[index].passport_number}</p>}
                        </div>
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => updatePassengerDocument(index, 'issuing_country_code', e.target.value)}
                            onBlur={() => handleBlur(index, 'issuing_country_code', true)}
                            className={`w-full px-4 py-2 border ${
                              errors[index]?.issuing_country ? 'border-red-500' : 'border-gray-300'
                            } rounded-lg focus:ring-2 focus:ring-[#55C3A9] focus:border-[#55C3A9] outline-none transition`}
                          >
                          ...
                          {errors[index]?.issuing_country && <p className="mt-1 text-sm text-red-600">{errors[index].issuing_country}</p>}
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => updatePassengerDocument(index, 'expires_on', e.target.value)}
                            onBlur={() => handleBlur(index, 'expires_on', true)}
                            min={getTodayDate()}
                            className={`w-full px-4 py-2 border ${
                              errors[index]?.passport_expiry ? 'border-red-500' : 'border-gray-300'
                            } rounded-lg focus:ring-2 focus:ring-[#55C3A9] focus:border-[#55C3A9] outline-none transition`}
                          />
                          {errors[index]?.passport_expiry && <p className="mt-1 text-sm text-red-600">{errors[index].passport_expiry}</p>}
                        </div>
                      </div>
                    </div>
                  </div>
